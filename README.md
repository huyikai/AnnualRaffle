...
6. **浏览器兼容性**：建议使用现代浏览器（Chrome、Firefox、Edge 等），需要支持 IndexedDB
7. **数据备份**：重要数据建议定期备份，可通过浏览器开发者工具导出：
   - localStorage 数据：Application → Local Storage
   - IndexedDB 数据：Application → IndexedDB → AnnualRaffle
8. **性能优化**：大量用户时建议压缩照片大小以提升加载速度
9. **奖项配置**：修改奖项只需编辑 `src/config/lottery.ts` 中的 `LOTTERY_ITEMS` 数组，所有相关配置会自动更新
10. **代码架构**：项目采用 Vue 3 Composition API 和 Composables 模式，业务逻辑封装在 `composables/` 目录中

...
export function annualRaffleHandler(
  ...
  if (category) {
    ...
    if (preset && preset.trim()) {
      ...
      if (validPresetIds.length >= num) {
        return validPresetIds.slice(0, num);
      }
      
      // 如果有效预设人员数量 < 奖项人数，使用所有有效预设人员，然后补充随机抽取
      if (validPresetIds.length > 0) {
        // 将有效预设人员添加到已中奖集合中，避免随机抽取时重复
        validPresetIds.forEach(id => wonSet.add(id));
        
        // 计算剩余需要抽取的名额
        const remaining = num - validPresetIds.length;
        
        // 预先计算可抽奖用户列表（排除已中奖和预设人员）
        ...
        if (availableUsers.length < remaining) {
          throw new Error(`可抽奖人数不足：预设名单已使用 ${validPresetIds.length} 人，还需要 ${remaining} 人，但只有 ${availableUsers.length} 人可抽`);
        }
        ...
        // 返回预设人员 + 随机抽取的人员
        return [...validPresetIds, ...randomResults];
      }
